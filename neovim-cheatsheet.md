...
### バッファ再読み込み
- `<leader>br` - 現在のバッファを再読み込み
- `<leader>bR` - 全バッファの変更をチェック
- `<F5>` - バッファを再読み込み（F5）
